...
function NumberStepper({ label, value, step = 1, min, max, onChange }) {
  ...
  const update = (next) => {
    const bounded = Math.max(min ?? -Infinity, Math.min(max ?? Infinity, Number(next)));
  ...
  return (
    <label className="grid min-w-0 gap-2 rounded-xl bg-slate-50 p-3">
      ...
      <div className="grid min-w-0 grid-cols-[40px_minmax(0,1fr)_40px] gap-2">
        <button type="button" className="rounded-lg bg-white text-lg font-black text-slate-700 ring-1 ring-slate-200" onClick={() => update(current - step)}>
          -
        </button>
        <input
          type="number"
...
function Quick3DControls({
  ...
  const isPath = selected.kind === "path" && activeAction?.path?.[selected.pathIndex];
  const pathPoint = isPath ? activeAction.path[selected.pathIndex] : null;

  const patchAction = (patch) => onUpdateAction(selected.segmentIndex, selected.actionIndex, patch);
  ...
  return (
    <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.14em] text-slate-500">Thông số nhanh</p>
          <h3 className="mt-1 text-xl font-black text-slate-950">
            {isAction ? activeAction.label || actionLabels[activeAction.type] : isMarker ? activeMarker.label : isPath ? `Điểm đường đi ${selected.pathIndex + 1}` : "Chọn mốc hoặc asset"}
          </h3>
        </div>
      ...
      {!isAction && !isMarker && !isPath ? (
        <p className="mt-4 text-sm leading-6 text-slate-500">
          Bấm trực tiếp vào mốc, asset hoặc điểm đường đi trên bản đồ 3D để chỉnh thông số tại đây.
        </p>
      ) : null}

      {isMarker ? (
        <div className="mt-4 grid gap-3">
          <input className={fieldClass()} value={activeMarker.label} onChange={(event) => onUpdateMarker(activeMarker.id, { label: event.target.value })} />
          <div className="grid gap-3">
            <NumberStepper label="X trên bản đồ (%)" value={activeMarker.x} step={0.1} min={0} max={100} onChange={(value) => onUpdateMarker(activeMarker.id, { x: value })} />
            <NumberStepper label="Y trên bản đồ (%)" value={activeMarker.y} step={0.1} min={0} max={100} onChange={(value) => onUpdateMarker(activeMarker.id, { y: value })} />
            <NumberStepper label="Độ cao Z" value={activeMarker.z} step={0.01} min={0} max={0.6} onChange={(value) => onUpdateMarker(activeMarker.id, { z: value })} />
            <NumberStepper label="Kích thước mốc" value={activeMarker.scale} step={0.05} min={0.05} max={5} onChange={(value) => onUpdateMarker(activeMarker.id, { scale: value })} />
          </div>
          <input className={fieldClass()} placeholder="Đường dẫn video khi bấm mốc" value={activeMarker.videoPath || ""} onChange={(event) => onUpdateMarker(activeMarker.id, { videoPath: event.target.value })} />
        </div>
      ) : null}

      {isAction ? (
        <div className="mt-4 grid gap-3">
          <input className={fieldClass()} value={activeAction.label || ""} onChange={(event) => patchAction({ label: event.target.value })} />
          <div className="grid gap-3">
            <NumberStepper label="Bắt đầu (giây)" value={activeAction.startAt} step={0.1} min={0} max={segmentDuration(activeSegment)} onChange={(value) => patchAction({ startAt: value })} />
            <NumberStepper label="Thời lượng hiện" value={activeAction.duration} step={0.1} min={0.1} max={segmentDuration(activeSegment)} onChange={(value) => patchAction({ duration: value })} />
            <NumberStepper label="X trên bản đồ (%)" value={activeAction.position?.x ?? 50} step={0.1} min={0} max={100} onChange={(value) => patchAction({ position: { ...activeAction.position, x: value } })} />
            <NumberStepper label="Y trên bản đồ (%)" value={activeAction.position?.y ?? 50} step={0.1} min={0} max={100} onChange={(value) => patchAction({ position: { ...activeAction.position, y: value } })} />
            <NumberStepper
              label="Độ cao Z"
          ...
          <label className="flex items-center gap-2 rounded-xl bg-slate-50 p-3 text-sm font-bold text-slate-700">
            <input type="checkbox" checked={Boolean(activeAction.transform?.followPathRotation)} onChange={(event) => onUpdateActionTransform("followPathRotation", event.target.checked)} />
      ...
      {isPath ? (
        <div className="mt-4 grid gap-3">
          <div className="grid grid-cols-3 gap-3">
            <NumberStepper label="X điểm path (%)" value={pathPoint.x} step={0.1} min={0} max={100} onChange={(value) => patchPathPoint({ x: value })} />
            <NumberStepper label="Y điểm path (%)" value={pathPoint.y} step={0.1} min={0} max={100} onChange={(value) => patchPathPoint({ y: value })} />
            <NumberStepper label="Z điểm path" value={pathPoint.z ?? transformValue(activeAction, "z", 0.08)} step={0.01} min={0} max={0.8} onChange={(value) => patchPathPoint({ z: value })} />
          </div>
        </div>
      ) : null}
    </div>
...
function VoiceTimeline({ segment, selectedActionIndex, playTime, playing, onPlay, onScrub, onSelectAction }) {
  ...
  return (
    <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.14em] text-slate-500">Timeline giọng đọc</p>
          <h3 className="text-xl font-black text-slate-950">{segment?.title || "Chưa có đoạn giọng đọc"}</h3>
        </div>
        <button type="button" onClick={onPlay} className="rounded-lg bg-slate-950 px-4 py-2 text-sm font-black text-white">
      ...
      <div className="relative mt-4 h-32 overflow-hidden rounded-xl bg-slate-100">
        <div className="absolute inset-y-0 w-0.5 bg-red-500" style={{ left: `${(playTime / duration) * 100}%` }} />
          ...
          return (
            <button
              ...
              className={`absolute top-4 h-12 rounded-lg px-3 text-left text-xs font-black shadow-sm ${
                selectedActionIndex === index ? "bg-slate-950 text-white" : isActive ? "bg-amber-300 text-slate-950" : "bg-white text-slate-700"
              }`}
              style={{ left: `${left}%`, width: `${Math.max(8, width)}%` }}
            >
              <span className="block truncate">{action.label || actionLabels[action.type] || action.type}</span>
              <span className="block text-[10px] opacity-70">
                {Number(action.startAt || 0).toFixed(1)}s - {(Number(action.startAt || 0) + Number(action.duration || 0)).toFixed(1)}s
              </span>
            </button>
          );
        })}
...
export default function TimelineEditor() {
  ...
  const [selected, setSelected] = useState({ kind: "marker", markerId: defaultConfig.markers[0].id, segmentIndex: 0, actionIndex: 0 });
  ...
  const activeSegment = state.segments[selected.segmentIndex] || state.segments[0];
  const activeAction = activeSegment?.actions?.[selected.actionIndex] || null;
  const activeMarker = state.markers.find((marker) => marker.id === selected.markerId) || state.markers[0];
  ...
  useEffect(() => {
    ...
    if (activeSegment?.audioPath) audioRef.current.src = mediaPathToUrl(activeSegment.audioPath);
  }, [activeSegment]);

  const patchState = (updater) => setState((current) => normalizeConfig(updater(current)));
  ...
  const updateAction = (segmentIndex, actionIndex, patch) => {
    patchState((current) => ({
      ...
      segments: current.segments.map((segment, currentSegmentIndex) => {
        ...
        return {
          ...segment,
          actions: segment.actions.map((action, currentActionIndex) => (currentActionIndex === actionIndex ? { ...action, ...patch } : action)),
        };
      }),
    }));
  };
  ...
  const updateActionTransform = (field, value) => {
    ...
    if (field === "z") {
      updateAction(selected.segmentIndex, selected.actionIndex, {
        ...
        path: (activeAction.path || []).map((point) => ({ ...point, z: value })),
  ...
  const addMarker = () => {
    ...
    patchState((current) => ({ ...current, markers: [...current.markers, marker] }));
    setSelected((current) => ({ ...current, kind: "marker", markerId: marker.id }));
  };

  const addSegment = () => {
    const segment = makeSegment(state.segments.length, state.markers[0]?.id || "");
    patchState((current) => ({ ...current, segments: [...current.segments, segment] }));
    setSelected((current) => ({ ...current, segmentIndex: state.segments.length, actionIndex: 0 }));
    setPlayTime(0);
  };
  ...
  const addAction = (type = "model") => {
    ...
    patchState((current) => ({
      ...
      segments: current.segments.map((segment, segmentIndex) =>
        segmentIndex === selected.segmentIndex ? { ...segment, actions: [...segment.actions, action] } : segment
      ),
    }));
    setSelected((current) => ({ ...current, kind: "action", actionIndex: activeSegment.actions.length }));
  };
    ...
    patchState((current) => ({
      ...
      segments: current.segments.map((segment, segmentIndex) =>
        segmentIndex === selected.segmentIndex
          ? { ...segment, actions: segment.actions.filter((_, index) => index !== selected.actionIndex) }
          : segment
      ),
    }));
    setSelected((current) => ({ ...current, actionIndex: Math.max(0, current.actionIndex - 1) }));
  };

  const handlePlacePoint = (point) => {
    if (mapMode === "marker" && activeMarker) {
      updateMarker(activeMarker.id, { x: point.x, y: point.y });
      setSelected((current) => ({ ...current, kind: "marker", markerId: activeMarker.id }));
    }
  ...
  const saveConfig = async () => {
    ...
    try {
      ...
      if (!response.ok || !result.ok) throw new Error(result.error || "Lưu thất bại");
      setSaveStatus("Đã lưu vào public/ar-config/ar-timeline-config.json. Tải lại trên điện thoại để dùng cấu hình mới.");
    } catch (error) {
    ...
    <div className="grid gap-5 text-slate-950">
      <div className="rounded-2xl bg-slate-950 p-4 text-white shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.14em] text-amber-200">Trình dựng AR lịch sử</p>
            <h2 className="mt-1 text-2xl font-black">Dựng voice, mốc và asset 3D trên bản đồ</h2>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">
              Chỉnh trên máy tính, lưu JSON, rồi mở URL bên dưới bằng điện thoại cùng mạng. Bản đồ 3D trong editor dùng cùng tọa độ với AR tracking.
            </p>
          </div>
          <div className="grid min-w-72 gap-2 rounded-xl bg-white/10 p-3">
            <p className="text-xs font-black uppercase tracking-[0.14em] text-slate-300">URL mở trên điện thoại</p>
            <input className="rounded-lg bg-white px-3 py-2 text-sm font-bold text-slate-950" readOnly value={mobileUrl || window.location.origin} />
            <button type="button" className="rounded-lg bg-amber-300 px-3 py-2 text-sm font-black text-slate-950" onClick={() => navigator.clipboard?.writeText(mobileUrl || window.location.origin)}>
              Sao chép URL
            </button>
          </div>
        </div>
        ...
        <div className="grid gap-5">
          <div className="rounded-2xl bg-white p-3 shadow-sm ring-1 ring-slate-200">
            <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.14em] text-slate-500">Chế độ đặt trên bản đồ</p>
                <p className="text-sm font-semibold text-slate-600">Click lên mặt bản đồ 3D hoặc kéo trực tiếp mốc/asset.</p>
              </div>
              <div className="flex flex-wrap gap-2">
                {Object.entries(mapModeLabels).map(([mode, label]) => (
                  <button key={mode} type="button" className={buttonClass(mapMode === mode)} onClick={() => setMapMode(mode)}>
                    {label}
                  </button>
                ))}
              </div>
            ...
            <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px] 2xl:grid-cols-[minmax(0,1fr)_400px]">
              <TimelineMap3DPreview
                ...
                mapMode={mapMode}
                playTime={playTime}
                onSelect={(next) => setSelected((current) => ({ ...current, ...next }))}
          ...
          <VoiceTimeline
            ...
            onSelectAction={(index) => setSelected((current) => ({ ...current, kind: "action", actionIndex: index }))}
          />
          ...
          <div className="grid gap-5 xl:grid-cols-[1fr_1fr]">
            <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <button type="button" onClick={addSegment} className="rounded-lg bg-slate-950 px-3 py-2 text-xs font-black text-white">
                  Thêm giọng đọc
                </button>
              </div>
              <div className="mt-3 grid gap-2 md:grid-cols-2">
                {state.segments.map((segment, index) => (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      setSelected((current) => ({ ...current, segmentIndex: index, actionIndex: 0 }));
                      setPlayTime(0);
                    }}
                  ...
                  >
                    ...
                    <p className="mt-1 truncate text-xs opacity-75">{segment.audioPath || "Chưa gắn file âm thanh"}</p>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <button type="button" onClick={removeAction} className="rounded-lg bg-red-500 px-3 py-2 text-xs font-black text-white">
                  Xóa asset
                </button>
              </div>
              <div className="mt-3 grid grid-cols-2 gap-2 md:grid-cols-3">
                {actionTypes.map((type) => (
                  <button key={type} type="button" className={buttonClass(activeAction?.type === type)} onClick={() => addAction(type)}>
                    Thêm {actionLabels[type] || type}
                  </button>
                ))}
              </div>
              {activeAction ? (
                <div className="mt-4 grid gap-3 md:grid-cols-2">
                  <label className="grid gap-1 text-xs font-black uppercase tracking-[0.12em] text-slate-500">
                    Loại asset
                    <select className={fieldClass()} value={activeAction.type} onChange={(event) => updateAction(selected.segmentIndex, selected.actionIndex, { type: event.target.value, assetPath: defaultAssetByType[event.target.value] || activeAction.assetPath })}>
                      {actionTypes.map((type) => <option key={type} value={type}>{actionLabels[type] || type}</option>)}
                    </select>
                  </label>
                  <label className="grid gap-1 text-xs font-black uppercase tracking-[0.12em] text-slate-500">
                    Đường dẫn GLB
                    <input className={fieldClass()} value={activeAction.assetPath || ""} onChange={(event) => updateAction(selected.segmentIndex, selected.actionIndex, { assetPath: event.target.value })} />
                  </label>
                </div>
              ) : null}
            </div>
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
          ...
          <div className="mt-4 grid gap-4">
            <div className="grid gap-3 rounded-xl bg-slate-50 p-3">
              <p className="text-xs font-black uppercase tracking-[0.12em] text-slate-500">Đoạn giọng đọc đang chọn</p>
              <input className={fieldClass()} value={activeSegment?.title || ""} onChange={(event) => updateSegment(selected.segmentIndex, { title: event.target.value })} />
              <input className={fieldClass()} placeholder="Đường dẫn audio" value={activeSegment?.audioPath || ""} onChange={(event) => updateSegment(selected.segmentIndex, { audioPath: event.target.value })} />
              <div className="grid grid-cols-2 gap-3">
                <label className="grid gap-1 text-xs font-black uppercase tracking-[0.12em] text-slate-500">
                  Thời lượng
                  <input type="number" className={fieldClass()} value={activeSegment?.duration || 0} onChange={(event) => updateSegment(selected.segmentIndex, { duration: Number(event.target.value) })} />
                </label>
                <label className="grid gap-1 text-xs font-black uppercase tracking-[0.12em] text-slate-500">
                  Mốc sáng sau giọng đọc
                  <select className={fieldClass()} value={activeSegment?.nextMarkerId || ""} onChange={(event) => updateSegment(selected.segmentIndex, { nextMarkerId: event.target.value })}>
                    <option value="">Không có</option>
                    {state.markers.map((marker) => <option key={marker.id} value={marker.id}>{marker.label}</option>)}
                  </select>
                </label>
              </div>
            </div>
            ...
            <div className="grid gap-3 rounded-xl bg-slate-50 p-3">
              <div className="flex items-center justify-between">
                <p className="text-xs font-black uppercase tracking-[0.12em] text-slate-500">Mốc trên bản đồ</p>
                <button type="button" onClick={addMarker} className="rounded-lg bg-slate-950 px-3 py-2 text-xs font-black text-white">
                  Thêm mốc
                </button>
              </div>
              <select className={fieldClass()} value={activeMarker?.id || ""} onChange={(event) => setSelected((current) => ({ ...current, kind: "marker", markerId: event.target.value }))}>
                {state.markers.map((marker) => <option key={marker.id} value={marker.id}>{marker.label}</option>)}
              </select>
              {activeMarker ? (
                <>
                  <input className={fieldClass()} value={activeMarker.label} onChange={(event) => updateMarker(activeMarker.id, { label: event.target.value })} />
                  <input className={fieldClass()} placeholder="Đường dẫn video khi bấm mốc" value={activeMarker.videoPath || ""} onChange={(event) => updateMarker(activeMarker.id, { videoPath: event.target.value })} />
                  <div className="grid grid-cols-4 gap-2">
                    <input type="number" step="0.1" className={fieldClass()} value={activeMarker.x} onChange={(event) => updateMarker(activeMarker.id, { x: Number(event.target.value) })} />
                    <input type="number" step="0.1" className={fieldClass()} value={activeMarker.y} onChange={(event) => updateMarker(activeMarker.id, { y: Number(event.target.value) })} />
                    <input type="number" step="0.01" className={fieldClass()} value={activeMarker.z} onChange={(event) => updateMarker(activeMarker.id, { z: Number(event.target.value) })} />
                    <input type="number" step="0.1" className={fieldClass()} value={activeMarker.scale} onChange={(event) => updateMarker(activeMarker.id, { scale: Number(event.target.value) })} />
                  </div>
                  <p className="text-xs font-bold text-slate-500">Theo thứ tự: X %, Y %, độ cao Z, kích thước mốc.</p>
                </>
              ) : null}
            </div>
            ...
            {activeAction ? (
              <div className="grid gap-3 rounded-xl bg-slate-50 p-3">
                <p className="text-xs font-black uppercase tracking-[0.12em] text-slate-500">Asset đang chọn</p>
                <input className={fieldClass()} value={activeAction.label || ""} onChange={(event) => updateAction(selected.segmentIndex, selected.actionIndex, { label: event.target.value })} />
                <select className={fieldClass()} value={activeAction.pointId || ""} onChange={(event) => updateAction(selected.segmentIndex, selected.actionIndex, { pointId: event.target.value })}>
                  {state.markers.map((marker) => <option key={marker.id} value={marker.id}>{marker.label}</option>)}
                </select>
                <div className="grid grid-cols-2 gap-2">
                  <label className="grid gap-1 text-xs font-black uppercase tracking-[0.12em] text-slate-500">
                    Xuất hiện ở giây
                    <input type="number" step="0.1" className={fieldClass()} value={activeAction.startAt} onChange={(event) => updateAction(selected.segmentIndex, selected.actionIndex, { startAt: Number(event.target.value) })} />
                  </label>
                  <label className="grid gap-1 text-xs font-black uppercase tracking-[0.12em] text-slate-500">
                    Hiện trong bao lâu
                    <input type="number" step="0.1" className={fieldClass()} value={activeAction.duration} onChange={(event) => updateAction(selected.segmentIndex, selected.actionIndex, { duration: Number(event.target.value) })} />
                  </label>
                </div>
                <div className="grid grid-cols-3 gap-2">
                  <input type="number" step="0.1" className={fieldClass()} value={activeAction.position?.x ?? 50} onChange={(event) => updateAction(selected.segmentIndex, selected.actionIndex, { position: { ...activeAction.position, x: Number(event.target.value) } })} />
                  <input type="number" step="0.1" className={fieldClass()} value={activeAction.position?.y ?? 50} onChange={(event) => updateAction(selected.segmentIndex, selected.actionIndex, { position: { ...activeAction.position, y: Number(event.target.value) } })} />
                ...
                <label className="flex items-center gap-2 text-sm font-bold text-slate-700">
                  <input type="checkbox" checked={Boolean(activeAction.transform?.followPathRotation)} onChange={(event) => updateActionTransform("followPathRotation", event.target.checked)} />
                  Tự xoay theo hướng di chuyển trên đường đi
                </label>
                <div className="rounded-lg bg-white p-3 text-xs font-bold text-slate-500">
                  Vị trí: {pointValue(activeAction.position)} | Số điểm đường đi: {activeAction.path?.length || 0}
                </div>
              </div>
            ) : null}
          </div>
        ...
        <div className="rounded-2xl bg-slate-950 p-4 text-white shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <button type="button" onClick={saveConfig} className="rounded-lg bg-amber-300 px-4 py-2 text-sm font-black text-slate-950">
              Lưu cấu hình
            </button>
          </div>
          {saveStatus ? <p className="mt-3 rounded-lg bg-white/10 px-3 py-2 text-sm text-amber-100">{saveStatus}</p> : null}
          <pre className="mt-4 max-h-[640px] overflow-auto rounded-xl bg-black/40 p-4 text-xs leading-5 text-slate-200">{jsonPreview}</pre>
        </div>
      </div>
    </div>
  );
